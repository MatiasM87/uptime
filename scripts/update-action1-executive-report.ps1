...
$summary = [pscustomobject]@{
  ...
  endpointsWithCriticalVulnerabilities = @($endpoints | Where-Object { (Get-CountValue $_.vulnerabilities "critical") -gt 0 }).Count
  criticalVulnerabilityFindings = [int](($endpoints | ForEach-Object { Get-CountValue $_.vulnerabilities "critical" } | Measure-Object -Sum).Sum)
